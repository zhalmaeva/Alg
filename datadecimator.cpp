...
DataDecimator::storage_t DataDecimator::douglas_peucker(const DataDecimator::storage_t& storage, double d)
{
    ...
    size_t h = static_cast<unsigned long>((std::prev(storage->end())->key - storage->begin()->key) / 400);
    ...
    while(!stack.empty()) {
        ...
        double k1 = (last->value - range.first->value) * norm;
        ...
        double l = std::sqrt(k1*k1 + k2*k2);
            ...
            auto dist = std::abs(k2 * norm * (it->value - range.first->value) - k1 * (it->key - range.first->key)) / l;
            if(dist > maxDist) {
            maxDist = dist;
            maxIt = it;
            }
        ...
        stack.pop();
        if(maxDist >= d) {
            if (std::distance(maxIt, range.second) > h)
                stack.push({ maxIt, range.second });
            dec_storage->add(*maxIt);
            if (std::distance(range.first, std::next(maxIt)) > h)
